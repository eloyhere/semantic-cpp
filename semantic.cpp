...
namespace semantic {
    ...
    std::map<D, Module> Statistics<E, D>::frequency() const {
        if (!frequencyCache.empty()) return frequencyCache;
    ...
    bool Semantic<E>::allMatch(const Predicate<E>& predicate) const {
        bool allMatch = true;
        (*generator)([&allMatch, &predicate](const E& element)-> void {
            if (!predicate(element)) {
                allMatch = false;
            }
            }, [&allMatch](const E& element)-> bool {
                return allMatch;
                }, [](const E& element, const Timestamp& index)-> Timestamp {
                    return index;
                    });
                return allMatch;
    }
    ...
    R Semantic<E>::collect(const Supplier<A>& supplier, const BiConsumer<A, E>& accumulator, const BiFunction<A, A, A>& combiner, const Function<A, R>& finisher) const {
        ...
        (*generator)([&container, &accumulator](const E& element)-> void {
            accumulator(container, element);
    ...
    Semantic<E> Semantic<E>::concat(const Semantic<E>& other) const {
        auto newGenerator = std::make_shared<Generator<E>>([this, other](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) {
            bool firstCompleted = false;
            (*generator)(accept, [&](const E& element)-> bool {
    ...
    void Semantic<E>::cout(const BiFunction<E, std::ostream&, std::ostream&>& formatter) const {
        (*generator)([&formatter](const E& element)-> void {
            formatter(element, std::cout);
            }, [](const E& element)-> bool {
                return false;
                }, [](const E& element, const Timestamp& index)-> Timestamp {
                    return index;
    ...
    void Semantic<E>::cout(const std::ostream& stream) const {
        ...
        (*generator)([&output](const E& element)-> void {
            output << element << " ";
            }, [](const E& element)-> bool {
                return false;
                }, [](const E& element, const Timestamp& index)-> Timestamp {
    ...
    void Semantic<E>::cout(const std::ostream& stream, const BiFunction<E, std::ostream&, std::ostream&>& formatter) const {
        std::ostream& output = const_cast<std::ostream&>(stream);
        (*generator)([&output, &formatter](const E& element)-> void {
            formatter(element, output);
            }, [](const E& element)-> bool {
                return false;
                }, [](const E& element, const Timestamp& index)-> Timestamp {
                    return index;
    ...
    Module Semantic<E>::count() const {
        Module count = 0;
        (*generator)([&count](const E& element)-> void {
            count++;
            }, [](const E& element)-> bool {
                return true;
                }, [](const E& element, const Timestamp& index)-> Timestamp {
    ...
    Semantic<E> Semantic<E>::distinct() const {
        ...
        auto newGenerator = std::make_shared<Generator<E>>([this, seen](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) mutable {
            (*generator)([&](const E& element)-> void {
    ...
    Semantic<E> Semantic<E>::distinct(const BiPredicate<E, E>& comparator) const {
        std::vector<E> seen;
        auto newGenerator = std::make_shared<Generator<E>>([this, seen, comparator](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) mutable {
            (*generator)([&](const E& element)-> void {
    ...
    Semantic<E> Semantic<E>::dropWhile(const Predicate<E>& predicate) const {
        bool dropping = true;
        auto newGenerator = std::make_shared<Generator<E>>([this, predicate, dropping](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) mutable {
            (*generator)([&](const E& element)-> void {
                if (dropping) {
                    if (!predicate(element)) {
                        dropping = false;
    ...
    void Semantic<E>::forEach(const Consumer<E>& consumer) const {
        (*generator)(consumer, [](const E& element)-> bool {
            return false;
            }, [](const E& element, const Timestamp& index)-> Timestamp {
                return index;
                });
    ...
    Semantic<E> Semantic<E>::filter(const Predicate<E>& predicate) const {
        auto newGenerator = std::make_shared<Generator<E>>([this, predicate](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) {
            (*generator)([&](const E& element)-> void {
                if (predicate(element) && accept) {
                    accept(element);
                }
                }, [&](const E& element)-> bool {
                    return predicate(element) && (!interrupt || interrupt(element));
                    }, redirect);
            });
        return Semantic<E>(newGenerator);
    }
    ...
    Semantic<E> Semantic<E>::flat(const Function<E, Semantic<E>>& mapper) const {
        auto newGenerator = std::make_shared<Generator<E>>([this, mapper](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) {
            (*generator)([&](const E& element)-> void {
    ...
    Semantic<R> Semantic<E>::flatMap(const Function<E, Semantic<R>>& mapper) const {
        auto newGenerator = std::make_shared<Generator<R>>([this, mapper](const Consumer<R>& accept, const Predicate<R>& interrupt, const BiFunction<R, Timestamp, Timestamp>& redirect) {
            (*generator)([&](const E& element)-> void {
                auto nestedStream = mapper(element);
                (*nestedStream.generator)(accept, interrupt, redirect);
                }, [](const E& element)-> bool { return true; }, [](const E& element, Timestamp ts)-> Timestamp { return ts; });
            });
        return Semantic<R>(newGenerator);
    }
    ...
    Semantic<E> Semantic<E>::limit(const Module& n) const {
        Module count = 0;
        auto newGenerator = std::make_shared<Generator<E>>([this, n, count](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) mutable {
            (*generator)([&](const E& element)-> void {
                if (count < n && accept) {
                    accept(element);
                    count++;
                    ...
                    return count < n && (!interrupt || interrupt(element));
                    }, redirect);
            });
        return Semantic<E>(newGenerator);
    }
        ...
        auto newGenerator = std::make_shared<Generator<R>>([this, mapper](const Consumer<R>& accept, const Predicate<R>& interrupt, const BiFunction<R, Timestamp, Timestamp>& redirect) {
            (*generator)([&](const E& element)-> void {
                ...
                if (accept) accept(result);
                    ...
                    return !interrupt || interrupt(mapper(element));
                    }, [&](const E& element, Timestamp ts)-> Timestamp {
                        R result = mapper(element);
    ...
    template<typename E>
    Semantic<E> Semantic<E>::offset(const Function<E, Timestamp>& n) const {
        return skip(1);
    }
    ...
    std::vector<std::vector<E>> Semantic<E>::partitionBy(const Function<E, Module>& classifier) const {
        std::vector<std::vector<E>> partitions;
        std::unordered_map<Module, std::vector<E>> partitionMap;

        (*generator)([&](const E& element)-> void {
            Module partitionKey = classifier(element);
            partitionMap[partitionKey].push_back(element);
            }, [](const E& element)-> bool {
                return true;
                }, [](const E& element, const Timestamp& index)-> Timestamp {
                    return index;
    ...
    Semantic<E> Semantic<E>::peek(const Consumer<E>& consumer) const {
        auto newGenerator = std::make_shared<Generator<E>>([this, consumer](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) {
            (*generator)([&](const E& element)-> void {
    ...
    template<typename E>
    Semantic<E> Semantic<E>::reindex(const BiFunction<E, Timestamp, Timestamp>& indexer) const {
        auto newGenerator = std::make_shared<Generator<E>>([this, indexer](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) {
            Timestamp currentIndex = 0;
            (*generator)(accept, interrupt, [&](const E& element, Timestamp index)-> Timestamp {
                return indexer(element, index);
                });
            });
        return Semantic<E>(newGenerator);
    }
        ...
        auto newGenerator = std::make_shared<Generator<E>>([this, predicate](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) {
            ...
            (*generator)([&](const E& element)-> void {
                if (taking && predicate(element) && accept) {
                    ...
                    return taking && predicate(element) && (!interrupt || interrupt(element));
                    }, redirect);
            });
        return Semantic<E>(newGenerator);
    }
    ...
    Semantic<E> from(const E* array, const Module& length) {
        return Semantic<E>(std::make_shared<Generator<E>>([array, length](const Consumer<E>& accept, const Predicate<E>& interrupt, const BiFunction<E, Timestamp, Timestamp>& redirect) {
            for (Module i = 0; i < length; i++) {
                ...
                Timestamp index = i;
                if (redirect) {
                    index = (redirect(element, i) % length ) + length;
                }
                if (accept) accept(element);
            }
            }));
...
int main() {
    ...
    auto s = semantic::from(array, 5).reindex([](const int& element, auto index)-> auto {
        return -index;
        });
    s.cout();
    return 0;
